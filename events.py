...
class Aimbot:
    ...
    def start(self):
        ...
        while True:
            ...
            with mss.mss() as sct:
                frame = np.array(sct.grab(detection_box))
                if self.collect_data:
            ...
            if len(results.xyxy[0]) != 0: #player detected
                ...
                for *box, conf, cls in results.xyxy[0]: #iterate over each player detected
                    ...
                    if not own_player:
                        corner_thickness = 2  # You can adjust the thickness of the lines
                        corner_length = 18
                        # Top-left corner
                        # Top-left corner
                        cv2.line(frame, (x1, y1), (x1 + corner_length, y1), (0, 20, 200), corner_thickness)
                        cv2.line(frame, (x1, y1), (x1, y1 + corner_length), (0, 20, 200), corner_thickness)

                        # Top-right corner
                        cv2.line(frame, (x2, y1), (x2 - corner_length, y1), (0, 20, 200), corner_thickness)
                        cv2.line(frame, (x2, y1), (x2, y1 + corner_length), (0, 20, 200), corner_thickness)

                        # Bottom-left corner
                        cv2.line(frame, (x1, y2), (x1 + corner_length, y2), (0, 20, 200), corner_thickness)
                        cv2.line(frame, (x1, y2), (x1, y2 - corner_length), (0, 20, 200), corner_thickness)

                        # Bottom-right corner
                        cv2.line(frame, (x2, y2), (x2 - corner_length, y2), (0, 20, 200), corner_thickness)
                        cv2.line(frame, (x2, y2), (x2, y2 - corner_length), (0, 20, 200), corner_thickness)
                        cv2.putText(frame, f"{int(conf * 100)}%", x1y1, cv2.FONT_HERSHEY_DUPLEX, 0.5, (244, 0, 0), 1) #draw the confidence labels on the bounding boxes
                    else:
                        own_player = False
                ...
                if closest_detection: #if valid detection exists
                    ...
                    if Aimbot.is_target_locked(absolute_head_X, absolute_head_Y):
                        cv2.putText(frame, "LOCKED", (x1 + 40, y1), cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 200, 0), 1) #draw the confidence labels on the bounding boxes
                    else:
                        cv2.putText(frame, "TARGETING", (x1 + 40, y1), cv2.FONT_HERSHEY_DUPLEX, 0.5, (0, 0, 255), 1) #draw the confidence labels on the bounding boxes
